...
export default function AdminLayoutPage() {
  ...
  const navItems = [
    ...
    { label: 'All Students', href: '/admin/students', icon: LuUsers },
    { label: 'Hub Management', href: '/admin/hubs', icon: LuLayoutGrid },
  ];
        ...
        <div className="border-t border-border p-4 space-y-2">
          <div className="flex items-center gap-3 px-4 py-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-xs font-medium text-foreground">{userProfile?.full_name || 'Admin'}</p>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium ${
                  location.pathname === item.href ? 'bg-primary text-white' : 'text-muted-foreground'
                }`}
              >
                <item.icon className="h-5 w-5" />
                {item.label}
